...
function _castToProperties(
    ...
    for (let [key, data] of Object.entries(bfDict)) {
        ...
        if (ffVersion && typeof data === 'string') {
            // Replace Firefox version in both rv:XXX.X and Firefox/XXX.X patterns
            data = data.replace(/(Firefox\/|rv:)(1[0-9]{2})\.([0-9]+)/g, `$1${ffVersion}.$3`);
            // Also handle standalone version numbers like "140.0" 
